        ...
        <div className="flex items-center gap-2">
          ...
          {shareLinks.map((link, i) => (
            <a
              ...
              className={`p-2 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 transition ${link.color} hover:text-white`}
              title={link.name}
              aria-label={`Partager sur ${link.name}`}
            >
              {link.icon ? <link.icon className="w-4 h-4" /> : link.svg}
            </a>
          ))}
          <button
            onClick={copyLink}
